...
import { profile, traits, sparkData, IMAGES } from "@/content";
  ...
  return (
    <section id="top" ref={ref} className="relative overflow-hidden pt-28 md:pt-36 pb-14" data-testid="hero-section">
      <div className="mx-auto max-w-[1400px] px-6 md:px-10">
            ...
            <FadeUp delay={0.75}>
              ...
              <div className="mt-10 flex flex-wrap items-center gap-5">
                ...
                <a
                  ...
                  className="group inline-flex items-center gap-2 border border-ink px-8 py-4 font-mono text-[11px] uppercase tracking-[0.25em] text-ink transition-colors duration-300 hover:bg-ink hover:text-white"
                >
                  {t("ctaCv")}
          ...
          <div className="relative col-span-12 mt-14 lg:col-span-5 lg:mt-0">
            ...
            >
              <div className="w-56 md:w-64 border border-ink bg-white p-5 shadow-[8px_8px_0_#050505]" data-testid="hero-forecast-card">
                <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-muted">{t("forecastLabel")}</p>
                ...
                <div className="mt-3 h-16">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={sparkData} margin={{ top: 2, right: 0, left: 0, bottom: 0 }}>
                      <Area type="monotone" dataKey="v" stroke="#1a73e8" strokeWidth={2} fill="#1a73e8" fillOpacity={0.12} isAnimationActive />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
              </div>
        ...
        <div className="mt-24 md:mt-28 grid grid-cols-1 sm:grid-cols-3 border-t border-line" data-testid="hero-traits">
          {traits.map((tr, i) => (
            <FadeUp key={tr.n} delay={0.15 * i} className={`border-line py-8 sm:pr-8 ${i < traits.length - 1 ? "sm:border-r" : ""} ${i > 0 ? "sm:pl-8" : ""} border-b sm:border-b-0`}>
              <div data-testid={`hero-trait-${i}`}>
                <p className="font-mono text-[11px] tracking-[0.2em] text-accent">{tr.n} /</p>
                <p className="mt-3 font-display text-3xl md:text-4xl text-ink">{pick(tr.text, lang)}</p>
              </div>
            </FadeUp>
          ))}
        </div>
        ...
        <FadeUp delay={0.4} className="mt-14 flex items-center gap-3">
          ...
          <span className="font-mono text-[10px] uppercase tracking-[0.3em] text-muted">{t("scrollHint")}</span>
        </FadeUp>
      </div>
    </section>
  );
